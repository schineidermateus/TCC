...
class Model(object):
    ...
    def get_data(self,rec_id,s,pitch_shift,scaling_factor):
        if self.mmap:
            ...
            f = open(fid, 'rb')
            f.seek(int(s*sz_float), os.SEEK_SET)
    ...
    def sample_records(self, rec_ids, count, fixed_stride=-1, pitch_shift=0):
        ...
        for i in range(len(rec_ids)):
            ...
            valid_indices = ~np.isnan(Y.flatten()) & ~np.isnan(this_Yhat.flatten())
            avp_breakdown[rec_ids[i]] = average_precision_score(Y.flatten()[valid_indices], this_Yhat.flatten()[valid_indices])
    ...
    def mse(self,X,Y):
        ...
        for j in range(subdiv):
            se += self.sess.run(self.loss_direct,
                           feed_dict={self.xd: X[int(subset*j):int(subset * (j + 1))],
                                      self.yd: Y[int(subset*j):int(subset * (j + 1))]})/subset
