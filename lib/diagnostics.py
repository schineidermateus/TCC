...
def mirex_statistics(model,record,threshold=.5,initial=False):
    ...
    valid_indices = ~np.isnan(Y.flatten()) & ~np.isnan(Yhat.flatten())
    avp = average_precision_score(Y.flatten()[valid_indices], Yhat.flatten()[valid_indices])
...
def pr_curve(model,record):
    ...
    valid_indices = ~np.isnan(Y.flatten()) & ~np.isnan(Yhat.flatten())
    P,R,_ = precision_recall_curve(Y.flatten()[valid_indices], Yhat.flatten()[valid_indices])
    plt.plot(R,P)
